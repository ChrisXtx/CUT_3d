...
class Downsample_3D(nn.Module):
    def __init__(self, channels, pad_type='reflect', filt_size=3, stride=2, pad_off=0):
        ...
        filt = get_filter_3D(filt_size=self.filt_size)
    ...
    def forward(self, inp):
        if(self.filt_size == 1):
            if(self.pad_off == 0):
                ...
                return F.pad(inp, self.pad_sizes, mode=self.pad_type)[:, :, ::self.stride, ::self.stride, ::self.stride]
        else:
            inp = self.pad(inp)
            out = F.conv3d(inp, self.filt, stride=self.stride, groups=inp.shape[1])
            return out
...
class Upsample_3D(nn.Module):
    def __init__(self, channels, pad_type='repl', filt_size=3, stride=2):
        ...
        filt = get_filter_3D(filt_size=self.filt_size) * (stride**2)
        self.register_buffer('filt', filt[None, None, :, :, :].repeat((self.channels, 1, 1, 1, 1)))
...
def get_norm_layer_3D(norm_type='group'):
    """Return a normalization layer
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'resnet_9blocks_3d':
        net = ResnetGenerator_3D(input_nc, output_nc, ngf, norm_layer=get_norm_layer_3D(norm_type=norm), use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
    elif netG == 'unet_128':
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netF == 'mlp_sample_3d':
        net = PatchSampleF_3D(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netD == 'basic_3d':  # more options
        net = NLayerDiscriminator_3D(input_nc, ndf, n_layers_D, norm_layer=get_norm_layer_3D(norm_type=norm), no_antialias=no_antialias,)
...
class ResnetGenerator_3D(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm3d or norm_layer.func == nn.GroupNorm
        else:
            use_bias = norm_layer == nn.InstanceNorm3d or norm_layer.func == nn.GroupNorm

        model = [nn.ReplicationPad3d(3),
                 nn.Conv3d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(int(ngf/2), ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if(no_antialias):
                model += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          norm_layer(int(ngf * mult * 2/2), ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                model += [nn.Conv3d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(int(ngf * mult * 2/2), ngf * mult * 2),
                          nn.ReLU(True),
                          Downsample_3D(ngf * mult * 2)]

        mult = 2 ** n_downsampling
        for i in range(n_blocks):       # add ResNet blocks

            model += [ResnetBlock_3D(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                model += [nn.ConvTranspose3d(ngf * mult, int(ngf * mult / 2),
                                             ...
                                             padding=1, output_padding=1,
                                             bias=use_bias),
                          norm_layer(int(ngf * mult / 4), int(ngf * mult / 2)),
                          nn.ReLU(True)]
            else:
                model += [Upsample_3D(ngf * mult),
                          ...
                          norm_layer(int(ngf * mult / 4), int(ngf * mult / 2)),
                          nn.ReLU(True)]
        padding_list = [3, 3, 3, 3, 3, 3]
        if opt.crop_depth < 80:
            padding_list = [3, 3, 3, 3, 3, 3]
        if opt.crop_size < 80: 
            padding_list = [2, 2, 2, 2, 2, 2]
        model += [nn.ReplicationPad3d(padding_list)]
...
class ResnetBlock_3D(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(int(dim/2), dim), nn.ReLU(True)]
        if use_dropout:
            conv_block += [nn.Dropout(0.5)]
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(int(dim/2), dim)]
...
class NLayerDiscriminator_3D(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        if(no_antialias):
            ...
            sequence = [nn.Conv3d(input_nc, ndf, kernel_size=kw, stride=1, padding=padw), nn.LeakyReLU(0.2, True), Downsample_3D(ndf)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            nf_mult_prev = nf_mult
            ...
            if(no_antialias):
                sequence += [
                    ...
                    norm_layer(int(ndf * nf_mult/2), ndf * nf_mult),
                    nn.LeakyReLU(0.2, True)
                ]
            else:
                sequence += [
